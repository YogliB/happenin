...
- Claude Code JSONL from `~/.claude/projects/<project>/<session>.jsonl`. Each `tool_use` block in an
  assistant message is recorded as its own event with `toolName` set (and `filePath` for
  `Read`/`Edit`/`Write`/`MultiEdit`/`NotebookEdit`, `skillName` for `Skill` calls), so tool usage,
  skills used, and files touched are queryable. `happenin sessions` also lists `tools`, `skills`, and
  `files` per session, and the dashboard's session detail view shows the same three lists.
...
Files are only re-parsed when their modification time changes. Pass `--force` to clear that tracking and re-import every discovered file from scratch — use this after upgrading `happenin` to pick up extraction changes for transcripts that were already imported.
